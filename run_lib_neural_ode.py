...
from models import ddpm, unet_class #ncsnv2, ncsnpp
import losses
import sampling, sampling_latent, sampling_spectral, sampling_neural_ode, sampling_predict, sampling_predict_multi, sampling_fkac
...
from utils import save_checkpoint, restore_checkpoint, restore_checkpoint_extended, save_checkpoint_extended
...
def train(config, workdir):
  ...
  model_spectral = mutils.create_model_spectral(config)
  ema_spectral = ExponentialMovingAverage(model_spectral.parameters(), decay=config.model.ema_rate)
  optimizer_spectral = losses.get_optimizer(config, model_spectral.parameters())
   
  state = dict(optimizer=optimizer,optimizer_spectral= optimizer_spectral, model=score_model, model_spectral = model_spectral, ema=ema, ema_spectral = ema_spectral, step=0)
  ...
  if config.training.sde.lower() == 'vpsde':
    ...
    sampling_eps = 1e-3
  elif config.training.sde.lower() == 'spectral_vpsde':
    sde = sde_lib.Spectral_VPSDE(beta_min=config.model.beta_min, beta_max=config.model.beta_max, N=config.model.num_scales)
    sampling_eps = 1e-3
  ...
  train_step_fn = losses.get_step_fn_spectral(sde, train=True, optimize_fn=optimize_fn,
                                     reduce_mean=reduce_mean, continuous=continuous,
                                     likelihood_weighting=likelihood_weighting, config = config)
  eval_step_fn = losses.get_step_fn_spectral(sde, train=False, optimize_fn=optimize_fn,
                                    reduce_mean=reduce_mean, continuous=continuous,
                                    likelihood_weighting=likelihood_weighting, config = config)

  # Building sampling functions
  if config.training.snapshot_sampling:
    if (config.training.forward_diffusion == 'predict' or config.training.forward_diffusion == 'predict_multi') and continuous:
      num_channels = config.data.num_channels//2
    else:
      num_channels = config.data.num_channels
    sampling_shape = (config.training.batch_size, num_channels,
                      config.data.image_size, config.data.image_size)
    if config.training.forward_diffusion == 'aniso' or config.training.forward_diffusion == 'spectral':
      sampling_fn = sampling_spectral.get_sampling_fn(config, sde, sampling_shape, inverse_scaler, sampling_eps) 
    elif config.training.forward_diffusion == 'predict':
      sampling_fn = sampling_predict.get_sampling_fn(config, sde, sampling_shape, inverse_scaler, sampling_eps) 
    elif config.training.forward_diffusion == 'fkac':
      sampling_fn = sampling_fkac.get_sampling_fn(config, sde, sampling_shape, inverse_scaler, sampling_eps)
    elif config.training.forward_diffusion == 'predict_multi':
      sampling_fn = sampling_predict_multi.get_sampling_fn(config, sde, sampling_shape, inverse_scaler, sampling_eps) 
    elif config.training.forward_diffusion == 'neural_ode' or config.training.forward_diffusion == 'neural_sde':
      sampling_fn = sampling_neural_ode.get_sampling_fn(config, sde, sampling_shape, inverse_scaler, sampling_eps)
  ...
  for step in range(initial_step, num_train_steps + 1):
    ...
    if step % config.training.log_freq == 0:
      logging.info("step: %d, training_loss: %.5e, kl_loss: %.5e, score_loss: %.5e," % (step, loss[0].item(), loss[1].item(), loss[2].item() ))
      writer.add_scalar("training_loss", loss[0], step)
      writer.add_scalar("kl_loss", loss[1], step)
    ...
    if step != 0 and step % config.training.snapshot_freq_for_preemption == 0:
      save_checkpoint_extended(checkpoint_meta_dir, state)
    ...
    if step % config.training.eval_freq == 0:
      ...
      # if (config.training.conditional_model == 'latent' and config.training.reconstruction_loss) or (config.training.conditional_model == 'latent_contrastive'):
      logging.info("step: %d, eval_loss: %.5e, eval_kl_loss: %.5e, eval_score_loss: %.5e" % (step, eval_loss[0].item(), eval_loss[1].item(), eval_loss[2].item()))
      writer.add_scalar("eval_loss", eval_loss[0].item(), step)
      writer.add_scalar("eval_kl_loss", eval_loss[1].item(), step)
      writer.add_scalar("eval_score_loss", eval_loss[2].item(), step)
      # elif (config.training.conditional_model == 'latent' and config.training.regularization=='highvar'):
    ...
    if step != 0 and step % config.training.snapshot_freq == 0 or step == num_train_steps:
      ...
      save_checkpoint_extended(os.path.join(checkpoint_dir, f'checkpoint_{save_step}.pth'), state)
      ...
      if config.training.snapshot_sampling:
        ...
        if config.training.conditional_model == 'latent':
          ...
          sample = sampling_fn(score_model, latent)
        elif config.training.conditional_model == 'latent_factor':
        ...
        if config.training.forward_diffusion == 'predict_multi':
          plot_samples_conditional(sample_dir, torch.cat((batch_orig.unsqueeze(-1),sample), dim=-1), step, [0,1])
        else:
          plot_samples_conditional(sample_dir, torch.cat((batch_orig.unsqueeze(-1),sample.unsqueeze(-1)), dim=-1), step, [0,1])
        scatter_plot_data(model_spectral, train_ds, config, scaler, viz_dir = sample_dir, epoch=step, n_ep=1000 )
...
def analyze(config, workdir, visualization_folder="viz"):
  ...
  sampling_shape = (config.eval.batch_size,
                    config.data.num_channels,
                    config.data.image_size, config.data.image_size)
  if config.training.forward_diffusion == 'predict':
    analysis_fn = sampling_predict.get_analysis_fn(config, sde, sampling_shape, inverse_scaler, sampling_eps)
  ...
  # step = (torch.FloatTensor([ckpt]) + 0.1*torch.FloatTensor([class_n])).item()
    ...
    # else:
    sample1, sample2, sample3 = analysis_fn(score_model, model_spectral, eval_batch_model, latent_position=pos_index)
...
def analyze_close(config, workdir, visualization_folder="viz"):
  ...
  # Build the sampling function when sampling is enabled
  num_channels = config.data.num_channels//2 if (config.training.forward_diffusion == 'predict' or config.training.forward_diffusion == 'predict_multi') and config.training.continuous else config.data.num_channels

  sampling_shape = (config.eval.batch_size,
                    num_channels,
                    config.data.image_size, config.data.image_size)
  if config.training.forward_diffusion == 'predict':
    analysis_fn = sampling_predict.get_analysis_fn(config, sde, sampling_shape, inverse_scaler, sampling_eps, type='close')
  elif config.training.forward_diffusion == 'fkac':
    analysis_fn = sampling_fkac.get_analysis_fn(config, sde, sampling_shape, inverse_scaler, sampling_eps, type='close')
  elif config.training.forward_diffusion == 'predict_multi':
    analysis_fn = sampling_predict_multi.get_analysis_fn(config, sde, sampling_shape, inverse_scaler, sampling_eps, type='close')
  elif config.training.forward_diffusion == 'neural_sde':
    analysis_fn = sampling_neural_ode.get_analysis_fn(config, sde, sampling_shape, inverse_scaler, sampling_eps, type='close')
  ...
  ckpt_path = os.path.join(checkpoint_dir, f'checkpoint_{ckpt}.pth')
  try:
    state = restore_checkpoint_extended(ckpt_path, state, device=config.device)
  except:
    time.sleep(60)
    try:
      state = restore_checkpoint_extended(ckpt_path, state, device=config.device)
  ...
  for batch_idx in range(4,5):
    ...
    if eval_batch.shape[2]!=config.data.image_size:
      eval_batch = resize_image(eval_batch, config.data.image_size )
    eval_batch = eval_batch.permute(0, 3, 1, 2)
    batch_size = eval_batch.shape[0]
    mode = 'project_one_hot' #'interpolate' #'project_one_hot' #'test_dim'#
    if mode == 'interpolate':
      eval_1 = eval_batch[batch_idx].repeat(batch_size//2,1,1,1)
      eval_2 = eval_batch[batch_idx+1].repeat(batch_size - batch_size//2,1,1,1)
      eval_batch = torch.cat((eval_1, eval_2), dim=0)
    else:
      eval_batch = eval_batch[batch_idx].repeat(batch_size,1,1,1)
    eval_batch_model = scaler(eval_batch)

    # else:
    # mode = 'project_one_hot' #'test_dim'#
    sampler = 'pc'#'sde'
    print(mode)
    if config.training.forward_diffusion == 'fkac':
      samples = analysis_fn(score_model, model_spectral, eval_batch_model, mode=mode, dim_idx=dim_idx, sampler=sampler, viz_dir=sample_dir, batch_idx = batch_idx)
      # samples = torch.cat((sample1.unsqueeze(-1), sample2.unsqueeze(-1), sample3.unsqueeze(-1)), dim =-1)
      samples=torch.cat((eval_batch.unsqueeze(-1),samples), dim=-1)
      plot_samples_analysis(sample_dir, samples, step, sampler+mode+'dim_'+str(dim_idx)+'_batch_'+str(batch_idx))
    else:
      samples, x_tilde_stack = analysis_fn(score_model, model_spectral, eval_batch_model, mode=mode, dim_idx=dim_idx, sampler=sampler, viz_dir=sample_dir, batch_idx = batch_idx)
        # samples = torch.cat((sample1.unsqueeze(-1), sample2.unsqueeze(-1), sample3.unsqueeze(-1)), dim =-1)
      samples_a=torch.cat((eval_batch.unsqueeze(-1),samples[:,:,:,:,0,:]), dim=-1)
      samples_b=torch.cat((eval_batch.unsqueeze(-1),samples[:,:,:,:,1,:]), dim=-1)
      samples_sum = torch.cat((eval_batch.unsqueeze(-1),samples[:,:,:,:,2,:]), dim=-1)
      plot_samples_analysis(sample_dir, samples_a, step, sampler+mode+'dim_'+str(dim_idx)+'_batch_'+str(batch_idx)+'_a')
      plot_samples_analysis(sample_dir, samples_b, step, sampler+mode+'dim_'+str(dim_idx)+'_batch_'+str(batch_idx)+'_b')
      plot_samples_analysis(sample_dir, samples_sum, step, sampler+mode+'dim_'+str(dim_idx)+'_batch_'+str(batch_idx)+'_sum')
      if config.training.continuous:
        plot_samples_analysis(sample_dir, x_tilde_stack[:,:,:,:,0,:], step, 'x_tilde_'+sampler+mode+'dim_'+str(dim_idx)+'_batch_'+str(batch_idx)+'_a')
        plot_samples_analysis(sample_dir, x_tilde_stack[:,:,:,:,1,:], step, 'x_tilde_'+sampler+mode+'dim_'+str(dim_idx)+'_batch_'+str(batch_idx)+'_b')
