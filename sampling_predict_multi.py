...
class EulerMaruyamaPredictor(Predictor):
  def __init__(self, sde, score_fn, probability_flow=False, mode=None):
    ...
    # if mode is not None:
    self.rsde = sde.reverse_predict_multi(score_fn, probability_flow)
...
class AncestralSamplingPredictor(Predictor):
  ...
  def vpsde_update_fn(self, x, t, latent):
    sde = self.sde
    timestep = (t * (sde.N - 1) / sde.T).long()
    ...
    score_ab = self.score_fn(x[:,:,:,:,:2], t, latent)
    x_sum = x[:,:,:,:,2]
    score_x = self.score_fn(x_sum[:,:,:,:,None].repeat(1,1,1,1,2), t, latent)
    score_sum = score_x.mean(-1).unsqueeze(-1)
    scores = torch.cat((score_ab, score_sum), dim = -1)


    x_mean = (x + beta[:, None, None, None, None] * scores) / torch.sqrt(1. - beta)[:, None, None, None, None]
    noise = torch.randn_like(x)
  ...
  def update_fn(self, x, t, latent):
    if isinstance(self.sde, sde_lib.VESDE):
      ...
      return self.vpsde_update_fn(x, t, latent)
...
def shared_predictor_update_fn(x, t, sde, model, predictor, probability_flow, continuous, latent=None, latent_x= None, mode=None):
  ...
  score_fn = mutils.get_latent_score_fn_predict_multi(sde, model, train=False, continuous=continuous, generate=True)
...
def get_pc_sampler(sde, shape, predictor, corrector, inverse_scaler, snr,
  ...
  def pc_sampler(model, model_spectral, latent):
    ...
    with torch.no_grad():
      ...
      # three slices of x and two slices of latent to be passed to the sampler
      x = x.unsqueeze(-1).repeat(1,1,1,1,3)
      latent = latent.unsqueeze(-1). repeat(1,1,2)
