...
def get_sampling_fn(config, sde, shape, inverse_scaler, eps):
  ...
  elif sampler_name.lower() == 'neural_ode':
    sampling_fn = get_neural_ode_sampler(sde=sde,
                                  shape=shape,
                                  inverse_scaler=inverse_scaler,
                                  denoise=config.sampling.noise_removal,
                                  eps=eps,
  ...
  elif sampler_name.lower() == 'neural_sde':
    sampling_fn = get_neural_sde_sampler(sde=sde,
                                  shape=shape,
                                  inverse_scaler=inverse_scaler,
                                  denoise=config.sampling.noise_removal,
                                  eps=eps,
...
class EulerMaruyamaPredictor(Predictor):
  def __init__(self, sde, score_fn, probability_flow=False, mode=None):
    ...
    # if mode is not None:
    self.rsde = sde.reverse_predict(score_fn, probability_flow)


  def update_fn(self, x, t, latent, latent_x):
    dt = -1. / self.rsde.N
    z = torch.randn_like(x)
    drift, diffusion = self.rsde.sde(x, t, latent, latent_x)
    x_mean = x + drift * dt
...
def shared_predictor_update_fn(x, t, sde, model, predictor, probability_flow, continuous, latent=None, latent_x= None, mode=None):
  ...
  return predictor_obj.update_fn(x, t, latent, latent_x)
...
def get_pc_sampler(sde, shape, predictor, corrector, inverse_scaler, snr,
  ...
  def pc_sampler(model, model_spectral, latent):
    ...
    with torch.no_grad():
      ...
      latent_x = model_spectral.module.latent_to_x(latent)

      # latent_x = latent_x.unsqueeze(0)
      # latent_x = torch.cat(x.shape[0]*[latent_x], dim = 0).to(x.device)

      for i in range(sde.N):
        t = timesteps[i]
        vec_t = torch.ones(shape[0], device=t.device) * t
        x, x_mean = corrector_update_fn(x, vec_t, model=model)
        x, x_mean = predictor_update_fn(x, vec_t, model=model, latent=latent, latent_x = latent_x)
...
def get_latent_analyzer(sde, shape, predictor, corrector, inverse_scaler, snr,
  ...
  corrector_update_fn = functools.partial(shared_corrector_update_fn,
                                          ...
                                          n_steps=n_steps,
                                          compositional=compositional)
  def generate_samples(model, x, timesteps, latent, latent_x, mix = False):
    if mix:
    ...
    for i in range(sde.N):
        ...
        vec_t = torch.ones(shape[0], device=t.device) * t
        x, x_mean = corrector_update_fn(x, vec_t, model=model)
        x, x_mean = predictor_fn(x, vec_t, model=model, latent=latent, latent_x = latent_x)
  ...
  def latent_analyzer(model, model_spectral, x_in, num_latent=3, latent_position=1):
    """ The PC sampler funciton.
    ...
    with torch.no_grad():
      ...
      latent_x_1 = model_spectral.module.latent_to_x(latent1)
      latent_x_2 = model_spectral.module.latent_to_x(latent2)
      latent_x_3 = model_spectral.module.latent_to_x(latent3)
      # latent = latent.unsqueeze(0)
      # latent = torch.cat(x.shape[0]*[latent], dim = 0).to(x.device)
      out1 = generate_samples(model, x, timesteps, latent1, latent_x_1)
      out2 = generate_samples(model, x, timesteps, latent2, latent_x_2)
      out3 = generate_samples(model, x, timesteps, latent3, latent_x_3)
...
class ReverseDrift_ode(nn.Module):
  ...
  def drift_fn(self, x, t):   
    score_fn = mutils.get_score_fn_neural_ode(self.sde, self.model, train=self.train, continuous=self.continuous, generate=True)
...
def get_latent_analyzer_close(sde, shape, predictor, corrector, inverse_scaler, snr,
  ...
  def generate_samples_sde(model, x, latent, w_diffusion = False):
    x_shape = x.shape
    if w_diffusion:
      sde_func = Reverse_sde_w_diffusion(model, latent, beta_min=sde.beta_0, beta_max=sde.beta_1, shape = x_shape, train = False, continuous = True )
    else:
      sde_func = Reverse_sde(model, latent, beta_min=sde.beta_0, beta_max=sde.beta_1, shape = x_shape, train = False, continuous = True )
  ...
  def get_two_interpolate(latent, n_interp=5):
    # batch_sz = latent.shape[0]
    z_0 = latent[0]
    z_1 = latent[-1]
    z_0 = z_0.repeat(latent.shape[0],1)
    
   
    z_1 = z_1.repeat(latent.shape[0],1)
    alpha = torch.linspace(0,1,n_interp).unsqueeze(0).unsqueeze(0).to(latent.device)
    out = z_0.unsqueeze(-1)*(1-alpha) + z_1.unsqueeze(-1)*alpha
    
    return out



  def latent_analyzer(model, model_spectral, x_in, n_interp = 10, mode = 'project_one_hot', dim_idx = 1, sampler = 'pc', viz_dir = None, batch_idx =5):
    """ The PC sampler funciton.
    ...
    with torch.no_grad():
      ...
      #   latent, _ = model.module.encode(x_in)
      # else:
      latent = model_spectral.module.encode(x_in)
      # latent1, latent2, latent3 = modify_latent(latent, latent_position)
      if mode == 'project_one_hot':
        ...
        latents_interp_stack = get_two_interpolate(latent, n_interp =n_interp)
      
      data_plot_proj_2D(latents_interp_stack[0].permute(1,0).cpu().numpy(),viz_dir=viz_dir,idx=mode+'_dim_'+str(dim_idx)+'_batch_'+str(batch_idx))

      for i in range(latents_interp_stack.shape[-1]):
        latent_i = latents_interp_stack[:,:,i].to(x.device)
        latent_x_i = model_spectral.module.latent_to_x(latent_i)
        print('latent:', i)
        # if sampler == 'ode':
        #   score_fn = mutils.get_latent_score_fn(sde, model, train=False, continuous=continuous, generate=True)
        # ode_func = ReverseDrift_ode(sde, model, score_fn, latent= latent_i)
        #   ode_func.update_latent(latent_i)

        #   out_i = generate_samples_ode(ode_func, x)
        if sampler == 'sde':
          out_i = generate_samples_sde(model, x, latent_i, w_diffusion = False)
        else:
          out_i = generate_samples(model, x, timesteps, latent_i, latent_x_i)
        if i >0:
          out_stack = torch.cat((out_stack,out_i.unsqueeze(-1)), dim=-1)
          x_tilde_stack = torch.cat((x_tilde_stack,latent_x_i.unsqueeze(-1)), dim=-1)
        else:
          out_stack = out_i.unsqueeze(-1)
          x_tilde_stack = latent_x_i.unsqueeze(-1)

    return out_stack, x_tilde_stack
...
def get_neural_ode_sampler(sde, shape, inverse_scaler,
                    denoise=False, rtol=1e-5, atol=1e-5,
  ...
  def denoise_update_fn(model, x):
    ...
    #   score_fn = mutils.get_compositional_score_fn(sde, model, train=False, continuous=True)
    # else:
    score_fn = mutils.get_score_fn_neural_ode(sde, model, train=False, continuous=True)
    # Reverse diffusion predictor for denoising
    predictor_obj = ReverseDiffusionPredictor(sde, score_fn, probability_flow=False)
    vec_eps = torch.ones(x.shape[0], device=x.device) * eps
    _, x = predictor_obj.update_fn(x, vec_eps)
  ...
  def ode_sampler(model, latent, z=None):
    """The probability flow ODE sampler with black-box ODE solver.
    ...
    with torch.no_grad():
      ...
      ode_func = ReverseDrift_ode(sde, model, latent, train = False, continuous = True)
      # ode_func = ReverseDrift_ode(sde, model, score_fn, latent= latent)
...
def get_neural_sde_sampler(sde, shape, inverse_scaler,
                    denoise=False, rtol=1e-5, atol=1e-5,
  ...
  def sde_sampler(model, latent, z=None):
    """The probability flow ODE sampler with black-box ODE solver.
