...
import losses
import sampling, sampling_multivariate, sampling_conditional, sampling_equal_energy, sampling_basis
...
def train(config, workdir):
  ...
  if config.training.snapshot_sampling:
    ...
    if config.training.conditional_model == 'basis':
      sampling_fn = sampling_basis.get_sampling_fn(config, sde, sampling_shape, inverse_scaler, sampling_eps)
  ...
  for step in range(initial_step, num_train_steps + 1):
    ...
    if step != 0 and step % config.training.snapshot_freq == 0 or step == num_train_steps:
      ...
      if config.training.snapshot_sampling:
        ...
        if config.training.conditional_model == 'basis':
          class_n_1 = save_step%3
          class_n_2 = (save_step+1)%3
          
          sample, n = sampling_fn(score_model, batch, [class_n_1, class_n_2])
        elif config.sampling.type == 'multivariate':
        ...
        elif config.sampling.type == 'conditional' or config.training.conditional_model == 'basis':
          plot_samples_conditional(sample_dir, sample, step, [111, class_n_1,class_n_2])
        else:
