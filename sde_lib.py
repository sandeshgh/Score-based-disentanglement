...
class SDE(abc.ABC):
  ...
  def reverse_switch_1_u(self, score_fn, probability_flow= False):
    """Create the reverse-time SDE/ODE.
    ...
    class RSDE(self.__class__):
      ...
      def sde(self, x, t, latent, alpha):
        ...
        # score_ind_list.append(score_mean)
        score_ind_list.append(score_mean + (1-alpha)*score_u_mean/3)
        score_stack = torch.cat(score_ind_list, dim=-1)
  ...
  def reverse_switch_2_u(self, score_fn, probability_flow= False):
    """Create the reverse-time SDE/ODE.
