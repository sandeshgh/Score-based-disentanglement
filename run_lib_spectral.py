...
import losses
import sampling, sampling_latent, sampling_spectral, sampling_predict
    ...
    if step != 0 and step % config.training.snapshot_freq == 0 or step == num_train_steps:
      ...
      if config.training.snapshot_sampling:
        ...
        if config.training.conditional_model == 'latent':
          ...
          sample, n = sampling_fn(score_model, model_spectral, latent)
        elif config.training.conditional_model == 'latent_factor':
...
def analyze_close(config, workdir, visualization_folder="viz"):
  ...
  # Build the sampling function when sampling is enabled
  num_channels = config.data.num_channels//2 if config.training.forward_diffusion == 'predict' else config.data.num_channels
  ...
  for batch_idx in range(7,12):
    ...
    mode = 'interpolate' #'project_one_hot' #'test_dim'#
    if mode == 'interpolate':
    ...
    plot_samples_analysis(sample_dir, x_tilde_stack, step, 'x_tilde_'+sampler+mode+'dim_'+str(dim_idx)+'_batch_'+str(batch_idx))
        # plot_samples_conditional(sample_dir, sample, 12.0, [0.5,0.7])
  # print("Done!")
