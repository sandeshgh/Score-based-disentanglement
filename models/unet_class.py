...
from .unet_free import ResBlockTimeEmbed, ImageSelfAttention, timestep_embedding
from . import utils
...
@utils.register_model(name='unet_class_conditioned')
class UNetTimeStepClassConditioned(nn.Module):
    ...
    def __init__(self, config, assert_shapes: bool = False):
        super().__init__()
        ...
        #                                                       {(len(p_dropouts) + 1)}'
        self.channels = channels = list(config.model.channels)
        self.kernel_sizes = kernel_sizes = list(config.model.kernel_sizes)
        self.strides = strides = list(config.model.strides)
        self.paddings = paddings = list(config.model.paddings)
        self.assert_shapes = assert_shapes
        self.num_classes = num_classes = config.model.num_classes
        self.time_embed_size = time_embed_size = config.model.time_embed_size
        self.class_embed_size = class_embed_size = config.model.class_embed_size
        self.p_dropouts = p_dropouts = list(config.model.p_dropouts)
        self.use_downsample = config.model.downsample
    ...
    def forward(self, x: torch.FloatTensor, t: torch.Tensor, c: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        time_embedding = self.time_embed(t.unsqueeze(-1))
    ...
    def forward_generate(self, x, t, c):
