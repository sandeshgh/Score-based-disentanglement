...
import losses
import sampling, sampling_multivariate, sampling_conditional, sampling_latent, sampling_label
...
def tensor_from_idx(eval_label, label_size = 10):
    m = eval_label.shape[0]
    label = torch.zeros(m,label_size)
    label[torch.arange(m), eval_label] = 1
    return label


def plot_samples(sample_dir, sample, step):
...
def plot_samples_conditional(sample_dir, sample_stack, step, class_n):
  ...
  for k in range(sample_stack.shape[-1]):
    ...
    if len(class_n)>1:
      ...
      image_name = "sample_"+str(class_n[0])+".png"
...
def train(config, workdir):
  ...
  W = compute_dct(n=10).to(config.device)
  ...
  for step in range(initial_step, num_train_steps + 1):
    ...
    label = torch.from_numpy(data['label']._numpy())
    label = tensor_from_idx(label).to(config.device)
    label = torch.mm(label,W)
    ...
    if step % config.training.eval_freq == 0:
      ...
      eval_label = torch.from_numpy(eval_data['label']._numpy())
      eval_label = tensor_from_idx(eval_label).to(config.device)
      eval_label = torch.mm(eval_label, W)
    ...
    if step != 0 and step % config.training.snapshot_freq == 0 or step == num_train_steps:
      ...
      if config.training.snapshot_sampling:
        ...
        if config.training.conditional_model == 'label':
          ...
          latent = torch.zeros(10)
          latent[class_n] = 1

          # class_n_2 = (save_step+1)%10
          # latent_2 = torch.zeros(10)
          # latent_2[class_n_2] = 1

          latent_stack = latent.unsqueeze(-1).to(config.device)
          latent_stack = torch.mm(W.T, latent_stack).squeeze()
        ...
        elif config.sampling.type == 'conditional' or config.training.conditional_model == 'label':
          plot_samples_conditional(sample_dir, sample.unsqueeze(-1), step, [class_n])
        else:
...
def visualize(config, workdir, visualization_folder="viz"):
  ...
  if config.training.conditional_model == 'equal_energy':
    ...
    sample, n = sampling_fn(score_model)
    plot_samples_conditional(sample_dir, sample, ckpt)
  else:
    sample, n = sampling_fn(score_model)
...
def analyze(config, workdir, visualization_folder="viz"):
  ...
  sampling_shape = (config.eval.batch_size,
                    ...
                    config.data.image_size, config.data.image_size)
  if config.training.conditional_model == 'label':
    analysis_fn = sampling_label.get_analysis_fn(config, sde, sampling_shape, inverse_scaler, sampling_eps)
  ...
  latent_stack = torch.cat((z1.unsqueeze(-1), z2.unsqueeze(-1)), dim=-1).to(config.device)
  latent_stack = torch.mm(W.T, latent_stack).squeeze()
  ...
  # else:
  sample1, sample2, sample3 = analysis_fn(score_model, latent_stack)
  samples = torch.cat((eval_batch.unsqueeze(-1), sample1.unsqueeze(-1), sample2.unsqueeze(-1), sample3.unsqueeze(-1)), dim =-1)
  # samples=torch.cat((eval_batch.unsqueeze(-1),samples), dim=-1)
  plot_samples_analysis(sample_dir, samples, step, str(i)+','+str(j))
        # plot_samples_conditional(sample_dir, sample, 12.0, [0.5,0.7])
  print("Done!")
