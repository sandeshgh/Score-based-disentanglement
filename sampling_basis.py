...
class EulerMaruyamaPredictor(Predictor):
  def __init__(self, sde, score_fn, probability_flow=False, mode=None):
    ...
    if mode == 'interpolate':
      self.rsde = self.sde.reverse_weighted_swap(score_fn, probability_flow)
    else:
      self.rsde = sde.reverse_weighted(score_fn, probability_flow)
    

  def update_fn(self, x, t, latent):
    batch, lat_idx = latent
    dt = -1. / self.rsde.N
    z = torch.randn_like(x)

    x_0 = x[:,:,:,:,0]
    drift_0, diffusion = self.rsde.sde(x_0, t, batch, 'sum')
    x_mean_0 = x_0 + drift_0 * dt
    x_0 = x_mean_0 + diffusion[:, None, None, None] * np.sqrt(-dt) * z[:,:,:,:,0]

    x_1 = x[:,:,:,:,1]
    drift_1, diffusion = self.rsde.sde(x_1, t, batch, lat_idx[0])
    x_mean_1 = x_1 + drift_1 * dt
    x_1 = x_mean_1 + diffusion[:, None, None, None] * np.sqrt(-dt) * z[:,:,:,:,1]

    x_2 = x[:,:,:,:,2]
    drift_2, diffusion = self.rsde.sde(x_2, t, batch, lat_idx[1])
    x_mean_2 = x_2 + drift_2 * dt
    x_2 = x_mean_2 + diffusion[:, None, None, None] * np.sqrt(-dt) * z[:,:,:,:,2]

    x_out = torch.cat((x_0.unsqueeze(-1), x_1.unsqueeze(-1), x_2.unsqueeze(-1)), dim=-1)
    x_mean = torch.cat((x_mean_0.unsqueeze(-1), x_mean_1.unsqueeze(-1), x_mean_2.unsqueeze(-1)), dim=-1)
...
def shared_predictor_update_fn(x, t, sde, model, predictor, probability_flow, continuous, compositional, latent=None, mode=None):
  ...
  # else:
  score_fn = mutils.get_score_fn_basis(sde, model, train=False, continuous=continuous, generate=False)
...
def get_pc_sampler(sde, shape, predictor, corrector, inverse_scaler, snr,
  ...
  def pc_sampler(model, batch, lat_index):
    ...
    with torch.no_grad():
      model.eval()
      # alpha = model.module.encode(batch)
      # Initial sample
      ...
      for i in range(sde.N):
        ...
        x, x_mean = predictor_update_fn(x, vec_t, model=model, latent = [batch, lat_index])
...
def get_latent_analyzer(sde, shape, predictor, corrector, inverse_scaler, snr,
  ...
  def latent_analyzer(model, batch):
    """ The PC sampler funciton.
