...
def get_analysis_fn(config, sde, shape, inverse_scaler, eps):
  """Create a sampling function.
...
class EulerMaruyamaPredictor(Predictor):
  def __init__(self, sde, score_fn, probability_flow=False, mode= None):
    ...
    if mode is not None:
        self.rsde = sde.reverse_mix(score_fn, probability_flow)
...
def get_latent_analyzer(sde, shape, predictor, corrector, inverse_scaler, snr,
  ...
  def latent_analyzer(model, latents):
    """ The PC sampler funciton.
    ...
    #   latent = model.module.encode(x_in)
      latent1, latent2 = latents[:,0].unsqueeze(0), latents[:,1].unsqueeze(0)
      # latent1, latent2, latents = latent1.to(x.device), latent2.to(x.device), latents.to(device)
      latents = latents.unsqueeze(0)
      latent1 = torch.cat(x.shape[0]*[latent1], dim = 0).to(x.device)
      latent2 = torch.cat(x.shape[0]*[latent2], dim = 0).to(x.device)
      latents = torch.cat(x.shape[0]*[latents], dim = 0).to(x.device)


      out1 = generate_samples(model, x, timesteps, latent1)
      out2 = generate_samples(model, x, timesteps, latent2)
      out3 = generate_samples(model, x, timesteps, latents, mix=True)
